...
- **O moderador aprova antes de ir ao ar.** `status`: `pending` (nasce assim) -> `approved` (aparece pra plateia, no voto e no quadro) -> `current` (a "na vez": grande pra todos, sem voto, UMA por palestra; a tela do moderador rebaixa a anterior pra `approved` antes de subir a nova, `features/question-status-plan.js`, o banco não garante isso) | `answered` (sai do quadro) | `rejected`; "devolver" = volta pra `pending` (de approved, current ou rejected). Só e-mail Google verificado da lista em `isModerator()` (hoje `gdgcampinascontato@gmail.com`) muda o `status`, entre os 5 estados. Plateia e quadro listam os estados públicos (`PUBLIC_QUESTION_STATUSES` = current + approved, consulta `status in [...]`, o repository vira `in` quando o filtro é array; um teste confere config x regra); cada pessoa lista as próprias (`uid == a dela`) e vê o estado de cada uma.
