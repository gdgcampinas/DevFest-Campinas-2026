...
- **TRAVA DE HORÁRIO (hoje DESLIGADA, pra testar em DEV em qualquer dia): LIGAR ANTES DO EVENTO.** Um interruptor em dois lugares que precisam ficar iguais (um teste confere): `windowEnforced()` em `firestore.rules` (a linha `return false; // TRAVA-DE-HORARIO`) e `enforceWindow` em `data/talk-questions.js`. Ligada:
...
- **Até 3 perguntas por pessoa por palestra** (`maxPerPerson`): espaço `<talkKey>#1..#3`; o id do documento tem que ser `<uid>_<entryKey>` (`idMatchesEntry`), senão dava pra furar o limite.
- **O moderador aprova antes de ir ao ar.** `status`: `pending` (nasce assim) -> `approved` (aparece pra plateia, no voto e no quadro) | `rejected` | `answered` (sai do quadro). Só e-mail Google verificado da lista em `isModerator()` (hoje `gdgcampinascontato@gmail.com`) muda o `status`. A plateia NÃO lista perguntas dos outros (lê o quadro `talk-boards`, ver "Leitura barata"); cada pessoa lista só as próprias (`uid == a dela`) e vê o estado de cada uma.
- **Voto:** 1 por pessoa por pergunta aprovada, só na janela, sem desfazer.
- **Tudo em `data/talk-questions.js`:** `enabled`, `maxLength` 280, `maxPerPerson`, `pollMs`, `mineRefreshMs`, `boardPublishMs`, `publishVotes`, `QUESTION_STATUS`. As regras espelham `maxLength`/`maxPerPerson` (testes conferem).

**Leitura barata (sessão 8, sem Blaze, cabe nas 50 mil leituras/dia do Spark):** a plateia e a TV NÃO consultam perguntas nem votos. O moderador (`features/question-moderation.js`) ouve as perguntas por listener (`listen`), conta os votos das aprovadas NO SERVIDOR (`countWhere`, `getCountFromServer`) a cada `boardPublishMs` (60 s) e quando o conjunto de aprovadas muda, e `features/board-publisher.js` (dual, testado com repositories falsos) grava `talk-boards/<talkKey>` (`{ edition, updatedAt, questions: [{ id, text, name, votes? }] }`) SÓ se a ordem mudou. Celular (`features/talk-questions.js`) e TV (`features/board-questions.js`) ouvem esse documento (`onSnapshot`, 1 leitura por regravação, só com o modal/TV aberto). O número de votos NÃO vai pro quadro por padrão (`publishVotes: false`, senão regrava a cada voto e passa a cota: teste de orçamento mostra 39 mil x 207 mil leituras/dia no pior caso); a plateia vê a ordem e "Votar/Votado" (votos guardados no navegador, `myVotesRepository`); só quem já perguntou na palestra relê as próprias perguntas (`myAskedRepository`, a cada `mineRefreshMs` enquanto alguma está pendente). Votos: só o moderador lista/conta (regras). **Consequência: a ordem que a plateia vê só anda com a tela de moderação aberta na palestra.** Repositories: `firestore-repository.js` ganhou `listen` e `countWhere`; `firestore-document-repository.js` (documento único por id) + `talk-boards-repository.js` (plateia) e `moderationBoardsRepository` (moderador). Teste de orçamento e do publicador em `DevFestIA/tools/questions/questions.test.js`.

**Regras do check-in e das avaliações (sessão 8):** `checkins`, `talk-feedback` e `event-feedback` exigem `idMatchesEntry` (id exatamente `<uid>_<chave>`), chave de palestra válida (`validTalkKey`) em check-in e avaliação de palestra, e a chave `event-end` (= `EVENT_FEEDBACK_KEY`, um teste confere) na avaliação do evento: sem isso uma pessoa criava check-ins e avaliações à vontade com chaves inventadas. A avaliação da palestra só passa depois do fim dela quando a trava de horário está ligada (`talkEnded`). Limite conhecido: o login é anônimo (um uid por navegador), então quem insistir ainda fraudaria; a defesa real seria o App Check. Testes: `DevFestIA/tools/questions/feedback-rules.test.js` e o kit `DevFestIA/tools/lib/rules-test-kit.js`, os dois arquivos de regras rodam juntos em `run-rules-tests.sh`.

**Testes de tela (sessão 8, jsdom):** `DevFestIA/tools/dom/*.dom.test.js` testam as features de tela (perguntas da plateia, moderação, quadro da sala, feedback da palestra com o check-in por QR, feedback do evento) em jsdom com repositories falsos (`DevFestIA/tools/lib/fake-question-world.js`), sem Firebase nem emulador. O harness (`DevFestIA/tools/lib/dom-harness.js`) carrega os scripts do site na mesma ordem e escopo global do navegador; as features já recebem tudo por parâmetro (`deps()`, `config`, `now`, `whenReady`), então o teste só injeta substitutos. Única dependência npm do repo: `jsdom`, só em `DevFestIA/tools/package.json` (o site em `docs/` segue sem build). Rodar: `npm ci --prefix DevFestIA/tools && node --test DevFestIA/tools/dom/*.test.js` (roda no CI, passo "Test the screens"). Escrever teste novo: usar `loadSite({ scripts: [...SITE_BASE, ...] })`, `settle()` pra esperar promises, `textOf()` pro texto visível (jsdom não tem `innerText`), e fechar a janela no fim (`test.after(() => window.close())`, senão os `setInterval` seguram o processo).

**Decidido, ainda não implementado (ver handoff "PRÓXIMO TRABALHO"):** fluxo SIMPLES (decisão do Renato, sessão 8; "na vez"/`current` e "devolver" foram feitos e descartados): pergunta (até 3, com check-in) -> moderador aprova/rejeita/reabre/respondida -> votos -> ordem por votos; sem Blaze; plano B sem TV = página Sala ao vivo no celular com QR fixo por sala + tela Palco do moderador.
...
2. **Moderação** (`moderacao.html?trilha=<id>`, interna, uma por sala; `components/question-moderation.js`, `features/question-moderation.js`): login Google; fila para aprovar, no ar, respondidas, rejeitadas; ouve as perguntas por listener e publica o quadro da palestra (ver "Leitura barata").
...
**Dois logins separados no mesmo site (`data/firebase-client.js`):** `window.firebaseClient` (plateia, uid anônimo, app padrão) e `window.moderatorClient` (moderador, conta Google, app nomeado "moderator", repositories em `data/moderation-repositories.js`). O Firebase guarda o login por app, então entrar como moderador NÃO troca a identidade da plateia no mesmo navegador (antes trocava e o banco recusava a pergunta, porque o check-in era do uid anônimo antigo). Rede de segurança: se o banco recusar pergunta/voto, `features/talk-questions.js` refaz o check-in do uid atual e tenta de novo uma vez; e `ensureAnonymousUid` sai de qualquer conta não anônima do app da plateia.
...
**Palestra fixada (`?palestra=<código>`, ex.: `0900.ia`)** no quadro da sala e na moderação: fixa aquela palestra em qualquer dia e horário (sem `?demo=`), aberta pra perguntas; código que não é da sala é ignorado e vale o relógio (`pinnedCode`, `findTalkSlotByCode` em `features/room-board.js`, testado). Serve pra testar em DEV sem depender da data.
...
**Teste de ponta a ponta com o código e as regras de verdade, sem tocar o banco:** `DevFestIA/tools/emulator/start.sh` sobe o Firebase local (Firestore 8085 + Auth 9099); abrir o site em `http://localhost:8080` (audiência, quadro) e `http://127.0.0.1:8080` (moderador) com `?emulador=1` (`docs/js/data/firebase-emulator.js`, só funciona em localhost/127.0.0.1; faixa roxa "EMULADOR"). Cada endereço local é uma origem com login próprio, então dá pra ter pessoas e o moderador ao mesmo tempo em abas diferentes. O login Google do moderador é falso (`?moderador=<e-mail>`, padrão o primeiro da lista). Os dados ficam sob o projeto `devfest-campinas` no emulador (não `demo-devfest`). Combinado com `?ensaio=HH:MM`, testa janela, aprovação, quadro e QR de verdade.
