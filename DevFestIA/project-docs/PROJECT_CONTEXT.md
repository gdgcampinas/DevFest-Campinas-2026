...
**Decidido, ainda não implementado (ver handoff "PRÓXIMO TRABALHO"):** fluxo SIMPLES (decisão do Renato, sessão 8; "na vez"/`current` e "devolver" foram feitos e descartados): pergunta (até 3, com check-in) -> moderador aprova/rejeita/reabre/respondida -> votos -> ordem por votos; sem Blaze; plano B sem TV = página Sala ao vivo no celular com QR fixo por sala + tela Palco do moderador; e o redesenho da leitura pra caber nas 50 mil leituras/dia do Spark (documento único por palestra, contagem agregada só na TV/moderação).
