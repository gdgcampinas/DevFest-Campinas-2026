 ...
 *
 * Entrada (tudo por parâmetro): `schedule`, `track` (a sala), `siteUrl`, `now`, e duas funções de chave já existentes no
 ...
 */
const roomCheckinUrl = (code, siteUrl) => `${siteUrl}grade.html?checkin=${code}`;
const roomRateUrl = (code, siteUrl) => `${siteUrl}grade.html?avaliar=${code}`;
const roomRateEventUrl = siteUrl => `${siteUrl}index.html?avaliar=1`;

function resolveRoomBoard({ schedule, track, siteUrl, now, keyOf, codeOf }) {
  ...
  const panels = [];
  if (finished) {
    panels.push({ id: "cdRate", kind: "rate", heading: active ? "Avalie a palestra anterior" : "Avalie esta palestra", title: finished.talks[track.id].title, hint: "Escaneie pra avaliar (seu check-in é registrado junto)", url: roomRateUrl(codeOf(finished, track.id), siteUrl) });
  }
  if (active) {
    panels.push({ id: "cdCheckin", kind: "checkin", heading: "Check-in nesta palestra", title: active.talks[track.id].title, hint: "Aponte a câmera do celular pro QR code pra fazer check-in", url: roomCheckinUrl(codeOf(active, track.id), siteUrl) });
  }
  if (eventOver) {
    panels.push({ id: "cdEvent", kind: "rate", heading: "Avalie o evento", title: "DevFest Campinas", hint: "Obrigado por participar! Conte como foi", url: roomRateEventUrl(siteUrl) });
  }
