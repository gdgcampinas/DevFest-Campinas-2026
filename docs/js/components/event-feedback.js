 ...
 * cada), indicação de 0 a 10 e textos opcionais. Puro template; a lista de
 ...
 */
function eventFeedbackMarkup({ phase, form, message = "" }) {
  if (phase === "closed") {
  ...
  if (phase === "rate") {
    const aspects = form.aspects
      .map(aspect => `<div class="aspect-row"><span class="aspect-label">${aspect.label}</span>${starRatingMarkup({ name: `aspect-${aspect.id}`, label: aspect.label })}</div>`)
      .join("");
    return `<form class="talk-feedback feedback-form" data-event-feedback-form>
      ...
      <p class="feedback-question">Por aspecto (opcional)</p>
      <div class="aspect-list">${aspects}</div>
      <p class="feedback-question">${form.nps.question}</p>
      ${npsScaleMarkup({ name: "event-nps", ...form.nps })}
