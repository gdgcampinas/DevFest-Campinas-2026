...
function agendaActionsMarkup({ count, whatsappUrl }) {
  return `
    <span class="agenda-actions-count">${count} ${count === 1 ? "palestra salva" : "palestras salvas"}</span>
    <button type="button" class="chip-btn" data-agenda-export data-track-event="agenda_export">${iconMarkup("download")}Exportar (.ics)</button>
    ...
    <button type="button" class="chip-btn" data-agenda-copy data-track-event="agenda_share" data-track-target="copy">${iconMarkup("link")}<span>Copiar link</span></button>`;
}
